...
#'
#' @description
#' Starting from a poset dominance matrix, computes fuzzy separation matrices using Product t-norm and Probabilistic-sum t-conorm.
#'
#' @param dom A square numeric matrix representing the dominance degrees between pairs of poset elements.
#' Row and column names of \code{dom} are interpreted as the labels of the poset elements.
#' \code{dom} can be computed by using functions such as [BLSDominance()], [BubleyDyerMRP()], or [ExactMRP()].
#'
...
#'
#' FSep <- FuzzySeparationProbabilistic(BLS, types = c("symmetric", "vertical"))
#'
#' @name FuzzySeparationProbabilistic
#' @export
FuzzySeparationProbabilistic <- function(dom, types = c("symmetric", "asymmetricLower", "asymmetricUpper", "vertical", "horizontal")) {
