#' @title Extracting the incomparability set of a poset element
#'
#' @description
...
#'
#' @param element A character string (the name of a single poset element).
#'
#' @return
#' A character vector containing the names of the poset elements incomparable with the input element.
#'
#' @examples
#' elems <- c("a", "b", "c", "d")
#'
...
IncomparabilitySetOf <- function(poset, element) {
  ...
  tryCatch({
    result <- .Call(C_IncomparabilitySetOf, poset@ptr, element)
    return(result)
